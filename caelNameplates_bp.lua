...
local ThreatUpdate = function(self, elapsed)
	...
	if self.elapsed >= 0.2 then
		...
		local minValue, maxValue = self.healthBar:GetMinMaxValues()
		local curValue = self.healthBar:GetValue()
		local hppct = ("%i"):format(curValue / maxValue * 100)
		if curValue < maxValue then
			self.healthBar.hppct:SetText(hppct.."%")
		else
			self.healthBar.hppct:SetText("")
		end
	end
end
...
local ColorCastBar = function(self, shielded)
	if shielded then
		self:SetStatusBarColor(0.75, 0.75, 0.75) --0.8, 0.05, 0)
...
local CreatePlate = function(frame)
	...
	newNameRegion:SetPoint("TOP", healthBar, "TOP", 0, 15)
	newNameRegion:SetFont(font, fontSize + 2, fontOutline)
	...
	healthBar.hppct = healthBar:CreateFontString(nil, "ARTWORK")
	healthBar.hppct:SetPoint("TOPRIGHT", healthBar, "TOPRIGHT", -2, 0)
	healthBar.hppct:SetFont(font, fontSize - 2, fontOutline)
	healthBar.hppct:SetTextColor(0.75, 0.75, 0.75)
	healthBar.hppct:SetShadowOffset(1.25, -1.25)
	...
	frame.elapsed = 0
	frame.maxHp = healthBar:GetValue()
